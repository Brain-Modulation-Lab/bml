function [ref,U] = bml_rereference_PLB20250502(cfg,raw)
...
if ~raw_has_nan && ismember(method,{'CAR','LAR','VAR'})
  ...
  elseif ismember(method,{'bipolar'})
    ...
    % changes 20250502 we should have as many channels as requested by the
    % table, which is not necessarily equal to the # chans in raw
    ref = raw;
    ref.label = strcat(reftable.label, '_', reftable.reference ); 
    
    for t=1:numel(raw.trial)
        ref.trial{t} = nan([height(reftable) size(ref.trial{t}, 2)]); 
        for j=1:height(reftable)
            ...
            i_raw_anode = ismember(raw.label,reftable.label(j));
            i_raw_cathode = ismember(raw.label,reftable.reference(j));
     
            ref.trial{t}(j,:) = ...
                raw.trial{t}(i_raw_anode,:) - ...
                raw.trial{t}(i_raw_cathode,:);
        end
    end
