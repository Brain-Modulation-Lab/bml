...
def parse_args():
    parser=argparse.ArgumentParser(description="This script calculates the pitch features of an audio wav file using the tell docker provided by Garcia Lab.")
...
def main():
    ...
    for filename in glob.glob(os.path.join(input_path, '*.wav')):
        ...
        csv_filename= os.path.join(output_path, "features_pitch.csv")        
        ...
        if "errorMessage" not in response: # save only if no errors
            pitch_scores = json.loads(response["body"])["scores"]
            pitch_data = dict(itertools.islice(json.loads(response["body"])["data"].items(),2)) # slice only pitch__pitch_series and pitch__sample_hz
            print(pitch_data)
            
            #convert dict to dataframe
            output = pd.DataFrame.from_dict({**pitch_scores,**pitch_data}, orient='index').transpose()
