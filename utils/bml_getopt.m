function val = bml_getopt(opt, key, default, emptymeaningful)
...
% 2017.10.27 AB: adapted from fieldtrip
%
...
elseif istable(opt)
  assert(height(opt)==1,'configuration tables should be a 1 row high');
  if ~any(strcmp(key, opt.Properties.VariableNames))
    val = default;
  else
    val = opt.(key);
    if iscell(val)
